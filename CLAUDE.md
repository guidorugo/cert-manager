...
- **Session cookies**: HttpOnly, SameSite=Lax, Secure by default (set `SESSION_COOKIE_SECURE=false` for HTTP).
...
- `SESSION_COOKIE_SECURE` - Require HTTPS for session cookies (default: true, set to false for HTTP)
