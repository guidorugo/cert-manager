...
- **CA export**: `/ca/<id>/download?format=pem|chain|key|pkcs12` (admin-only). `key` serves the decrypted private key PEM (audit: `download_ca_private_key`); `pkcs12` requires a `password` parameter and bundles cert+key+parent chain (audit: `export_ca_pkcs12`), importable back via the PKCS#12 import. key/pkcs12 refuse certificate-only CAs.
