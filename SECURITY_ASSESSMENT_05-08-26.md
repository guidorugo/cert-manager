...
| A5 | Medium | ✅ **Accepted** | Subscriber private-key escrow — server-generated keys stored **encrypted** (Fernet/PBKDF2, no plaintext, not cached). Accepted for operator-managed/server certs; use the **CSR flow** for client/email/code-signing (SoftHSM protects CA keys, **not** leaf keys) | Key Mgmt |
...
**Deployment note:** the app is designed to run **behind a TLS-terminating reverse proxy** (nginx/apache), which satisfies the transport requirement for these routes — so A7's cleartext-HTTP limb is a **deployment responsibility met by the proxy**, not an app defect. The remaining app-level residual is that a downloaded key is still **unencrypted PEM at rest** with **no dual control** (D4). The secret-in-GET, empty-password, and GET-reachable limbs were fixed in v1.0.1/v1.1.0, and HSM-backed CA keys are now **non-exportable** (v2.0) — closing A7 entirely for those.

### [MEDIUM] A2 — Application database is world-readable on the host — *Revised (was High)*
`data/cert-manager.db` is mode `0644` (root); a stale `instance/cert-manager.db` (Feb, not gitignored) is also `0644`. These hold the Fernet-encrypted CA keys, password hashes, and audit log. Any local user or co-located container reading the path can copy them (feeds A1). **Revised to Medium** because it is purely a local filesystem-permissions issue on what appears to be a single-operator host with non-default secrets; **it rises to High on any shared/multi-tenant host.** **Fix:** `chmod 600` DB + `.env`; own by a non-root service UID; `chmod 700 data/`; delete the stale `instance/` DB and gitignore `instance/`; encrypt backups. **Mitigated (v1.1.0 / v2.0.1):** the live DB is `600` and `data/` `700` (entrypoint `umask 077` + `chmod`), `MASTER_PASSPHRASE` moved to a Docker secret, and the stale `instance/cert-manager.db` was **git-removed and `instance/` gitignored** in v2.0.1 (verified gone; it remains only in git *history*). **Residual:** the container still runs as **root** (H1), so files are root-owned — this defeats a non-root host reader but not a co-located root container; a non-root service UID and encrypted backups remain.
...
### [MEDIUM] A5 — Subscriber private-key escrow — ✅ *Accepted (operator-managed; use CSR for non-server certs)*
...
**Accepted (2026-08-06):** for this operator-managed / homelab CA, escrow of server-generated keys is an accepted convenience. Storage is sound and was verified: keys are **Fernet-encrypted** (PBKDF2-HMAC-SHA256, 600k iters, per-record salt), **never persisted in plaintext** (only `Certificate.private_key_enc`), and **not cached in memory** (the sole key cache is CA-only, for OCSP); the only in-memory exposure is the unavoidable transient plaintext during generation/download (**F1**). **Caveat:** for **client-auth, S/MIME, and code-signing** certificates — where the subscriber must exclusively hold the key — use the **CSR flow** (the app signs a CSR and never sees the private key). SoftHSM (v2.0) protects the **CA** key, **not** subscriber keys, so it does not remove leaf escrow.
...
- **A2:** ✅ DB `600` / `700` + passphrase Docker secret (v1.1.0), stale `instance/` DB removed (v2.0.1). *Remaining:* run as a non-root UID (H1); encrypt backups.
